...
export default function CreateUser() {
  ...
  const { register, handleSubmit, watch, formState} = useForm<CreateFormData>({
    resolver: yupResolver(createFormSchema)
  });
  const { errors } = formState

  const handleCreateUser: SubmitHandler<CreateFormData> = async (values, event) => {
        ...
        <Box as="form" flex="1" borderRadius={8}  bg="gray.800" p={["6", "8"]} onSubmit={handleSubmit(handleCreateUser)}>
          ...
          <VStack spacing="8">
            ...
            <SimpleGrid minChildWidth="240px" spacing={["6", "8"]} w="100%">
              ...
              <Input name="passwordConfirmation" type="password" label="Confirmação da senha" {...register("passwordConfirmation")} error={errors.passwordConfirmation}/>
            </SimpleGrid>
          </VStack>
